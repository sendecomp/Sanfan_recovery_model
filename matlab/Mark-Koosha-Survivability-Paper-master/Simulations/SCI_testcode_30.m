...
All_states = zeros(500, num_components)
for i = 0:46
...
for state_case = 1:length(All_states)
    ...
    if sum(All_states(state_case, 1:40)) <17
                continue;
            end
    while j <= FOM_LENGTH
            %% Inject failures:
            % Transmission lines
            Line.store(1:41, end) = All_states(state_case, 1:41);
            % FACTS devices
            sssc_state = All_states(state_case, 42:46);
            j = j+1;
            try
    ...
    Line.store(:, end) = ones(41, 1);
    Sssc.store(:, end) = ones(5, 1);
end
